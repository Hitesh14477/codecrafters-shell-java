...
//             // pipelines get special-cased and delegated straight to a real shell, see PipelineRunner
//             if (tokens.contains("|")) {
//                 try {
...
//                 }
//             } else if (command.equals("jobs")) {
//                 // same listing logic as the prompt-time reaper, just triggered manually by the user typing "jobs"
...
//  * reaping finished tasks at the prompt, and listing jobs for the `jobs` builtin.
//  */
// class JobManager {
...
//     // builds the listing for ALL current jobs (running and done), as printed by the `jobs` command;
//     // removes any that are done while we're at it. Used both for direct printing and for the
...
//     // prints the result of listJobs() directly to stdout, used by the `jobs` builtin
...
// /**
//  * Builtin command helpers: checking whether a name is a builtin, and running a builtin
//  * so its output can be captured as a string (needed for builtins acting as one stage of
//  * a pipeline, so output can be piped onward). Note: in this shell, pipelines are always
//  * delegated wholesale to a real shell (see PipelineRunner), so this is currently unused
//  * but kept for parity with the original implementation.
//  */
// class Builtins {

//     // keeping this as static so a builtin running "inside" a pipeline can see/update
...
//     // exposes the (possibly updated) working dir after a pipeline-builtin call, e.g. for `cd`
...
class JobManager {
    ...
    // in the middle of exiting (e.g. a reader that just received EOF from a pipe/FIFO
